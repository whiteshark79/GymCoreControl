                                    ...
                                    <table class="table table-bordered table-sm table-hover" id="dtTable">
                                        ...
                                        <tbody v-if="arrayVenta.length">
                                            <tr v-for="venta in arrayVenta" :key="venta.id">
                                                <td v-text="venta.fecha_hora" scope="row" data-label="FECHA"></td>
                                                <td data-label="CLIENTE">{{venta.nombre}}</td> 
                                                <td data-label="DOCUMENTO"><span class="text-mini">{{venta.tipo_comprobante}}</span> : {{ venta.serie_comprobante ? venta.serie_comprobante+'-'+venta.num_comprobante : '000-000' }} </td>
                                                <td align="right" data-label="SUBTOTAL">$ {{ (venta.total-venta.impuesto*venta.total).toFixed(2) }}</td>
                                                <td align="right" data-label="IVA">$ {{ (venta.impuesto*venta.total).toFixed(2) }}</td>
                                                <td align="right" data-label="TOTAL">$ {{ venta.total }}</td>
                                                <td align="right" data-label="ABONO">$ {{ venta.abono }}</td>
                                                <td align="right" data-label="SALDO">$ {{ (venta.total-venta.abono).toFixed(2) }}</td>
