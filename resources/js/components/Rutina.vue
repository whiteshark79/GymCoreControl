                            ...
                            <div class="form-group row justify-content-between">
                                <div class="row col-sm-12 col-md-12">
                                    ...
                                    <div class="input-group input-group-sm col-sm-1 col-md-1">                                     
                                        <select class="form-control col-sm-6 col-md-12" v-model="paginado" @change="listarRutina(1,buscar,criterio,paginado,ordenado,ascdesc)">
                                        ...
                                        <tr align="center">
                                            <th scope="col" width="5%">#
                                                <template v-if="(ordenado !== 'id' && ascdesc === 'asc') || (ordenado === 'id' && ascdesc === 'desc') ">
                                                    <a href="#" @click="listarRutina(1,buscar,criterio,paginado,'id','asc')"><span style="float:right"><i class="fas fa-arrow-down fa-xs"></i></span></a>
                                                </template>
                                                <template v-if="(ordenado !== 'id' && ascdesc === 'desc') || (ordenado === 'id' && ascdesc === 'asc')">
                                                    <a href="#" @click="listarRutina(1,buscar,criterio,paginado,'id','desc')"><span style="float:right"><i class="fas fa-arrow-up fa-xs"></i></span></a>
                                                </template>
                                            </th>
                                            <th scope="col" width="30%">NOMBRE
                                                <template v-if="(ordenado !== 'nombre' && ascdesc === 'asc') || (ordenado === 'nombre' && ascdesc === 'desc')">
                                                    <a href="#" @click="listarRutina(1,buscar,criterio,paginado,'nombre','asc')"><span style="float:right"><i class="fas fa-arrow-down fa-xs"></i></span></a>
                                                </template>
                                                <template v-if="(ordenado !== 'nombre' && ascdesc === 'desc') || (ordenado === 'nombre' && ascdesc === 'asc')">
                                                    <a href="#" @click="listarRutina(1,buscar,criterio,paginado,'nombre','desc')"><span style="float:right"><i class="fas fa-arrow-up fa-xs"></i></span></a>
                                                </template>
                                            </th>
                                            <th scope="col">DESCRIPCIÓN</th>
                                            <th scope="col" width="10%">ESTADO
                                                <template v-if="(ordenado !== 'condicion' && ascdesc === 'asc') || (ordenado === 'condicion' && ascdesc === 'desc')">
                                                    <a href="#" @click="listarRutina(1,buscar,criterio,paginado,'condicion','asc')"><span style="float:right"><i class="fas fa-arrow-down fa-xs"></i></span></a>
                                                </template>
                                                <template v-if="(ordenado !== 'condicion' && ascdesc === 'desc') || (ordenado === 'condicion' && ascdesc === 'asc')">
                                                    <a href="#" @click="listarRutina(1,buscar,criterio,paginado,'condicion','desc')"><span style="float:right"><i class="fas fa-arrow-up fa-xs"></i></span></a>
                                                </template>
                                            </th>
                                            <th scope="col" width="10%">ACCIONES</th>
                                        </tr>
                                    </thead>
                                    <tbody v-if="arrayRutina.length">
                                        <tr v-for="rutina in arrayRutina" :key="rutina.id">
                                            <td v-text="rutina.id" align="center" scope="row" data-label="#"></td>
                                            <td v-text="rutina.nombre" data-label="NOMBRE"></td>
                                            <td v-text="rutina.descripcion" data-label="DESCRIPCIÓN"></td>                                            
                                            <td align="center" data-label="ESTADO">
                                                <div v-if="rutina.condicion"><span class="badge badge-success">Activo</span></div>
            ...
            <div class="modal-dialog modal-primary modal-sm modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" v-if="tipoAccion==1" class="btn btn-info btn-sm" @click="registrarRutina()">Guardar</button>                        
                        <button type="button" v-if="tipoAccion==2" class="btn btn-success btn-sm" @click="actualizarRutina()">Actualizar</button>
...
<script>
    export default {
        ...
        methods : {
            listarRutina (page,buscar,criterio,paginado,ordenado,ascdesc){
                let me=this;
                var url= '/rutina?page='+page+'&buscar='+buscar+'&criterio='+criterio+'&paginado='+paginado+'&ordenado='+ordenado+'&ascdesc='+ascdesc;
                axios.get(url).then(function (response) {
                    var respuesta= response.data;
                    me.arrayRutina = respuesta.rutinas.data;
            ...
            cambiarPagina(page,buscar,criterio,paginado,ordenado,ascdesc){
                ...
                me.listarRutina(page,buscar,criterio,paginado,ordenado,ascdesc);
            },
            registrarRutina(){
               if (this.validarRutina()){ return; }
                let me = this;

                axios.post('/rutina/registrar',{
                    'nombre': this.nombre,
                    ...
                    me.listarRutina(me.pagination.current_page,'','nombre',me.paginado,me.ordenado,me.ascdesc);
                }).catch(function (error) {
                    console.log(error);
                });
            },
            ...
            validarRutina(){
                this.errorRutina=0;
                this.errorMostrarMsjRutina =[];

                if (!this.nombre) {this.e_nombre = true; this.errorMostrarMsjRutina.push('nombre');}else{this.e_nombre = false}
                if (!this.descripcion) {this.e_descripcion = true; this.errorMostrarMsjRutina.push('descripcion');}else{this.e_descripcion = false}

                if (this.errorMostrarMsjRutina.length) this.errorRutina = 1;
                return this.errorRutina;
            },
            cerrarModal(){
                this.modal=0;
                this.tituloModal='';
                ...
                switch(modelo){
                    case "rutina":
                    {
                        switch(accion){
                            ...
                            {
                                this.modal = 1;
                                this.tituloModal = 'Registrar Rutina';
                                this.nombre= '';
                                this.descripcion = '';
                                this.tipoAccion = 1;
                                break;
                            ...
                            {
                                this.modal=1;
                                this.tituloModal='Actualizar rutina';
                                this.tipoAccion=2;
                                this.rutina_id=data['id'];
            ...
            ceroBusqueda(){
                this.buscar='';
                this.listarRutina(1,this.buscar,this.criterio,this.paginado,this.ordenado,this.ascdesc);
            }
        },
        mounted() {
            this.listarRutina(1,this.buscar,this.criterio,this.paginado,this.ordenado,this.ascdesc);
