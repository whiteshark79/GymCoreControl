                        ...
                        <div class="card-header">
                            ...
                            <div class="card-tools">
                                <template v-if="listado!=0">
                                    <button type="button" class="btn btn-sm btn-primary" @click="mostrarDetalle()"><i class="icon-plus">  Nueva Compra</i></button>
                                </template>
                            </div>
                        </div>                      
    ...
    export default {
        ...
        methods : {
            ...
            validarIngreso(){
                ...
                if (this.idproveedor==0) {this.e_idproveedor = true; this.errorMostrarMsjIngreso.push('idproveedor');}else{this.e_idproveedor = false}

                if(this.tipo_comprobante == 'FACTURA')
                {
                    if (this.serie_comprobante==0) {this.e_serie_comprobante = true; this.errorMostrarMsjIngreso.push('serie_comprobante');}else{this.e_serie_comprobante = false}
                    if (this.num_comprobante==0) {this.e_num_comprobante = true; this.errorMostrarMsjIngreso.push('num_comprobante');}else{this.e_num_comprobante = false}
                    if (!this.impuesto) {this.e_impuesto = true; this.errorMostrarMsjIngreso.push('impuesto');}else{this.e_impuesto = false}
                }

                if (this.arrayDetalle.length<=0) {this.e_detalle = true; this.errorMostrarMsjIngreso.push('detalle');}else{this.e_detalle = false} 
